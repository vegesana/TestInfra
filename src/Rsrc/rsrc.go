...
func (rsc *ResourceMgmt) ResourceRoutine() {
	...
	for {
		...
		case reqResource := <-rsc.RsrcGetCh:
			// Input should be response channel and count, if not enough
			// resources avilable, we return nil
			if reqResource.count <= rsc.RsrcCount {
			}
